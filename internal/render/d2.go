...
func (r *D2Renderer) Render(infra *model.Infrastructure, cfg *config.Config) string {
	...
	for _, stype := range groupOrder {
		...
		b.WriteString(fmt.Sprintf("    style.fill: %q\n", color.Fill))
		b.WriteString(fmt.Sprintf("    style.stroke: %q\n", color.Stroke))
		b.WriteString("\n")
...
func (r *D2Renderer) renderServer(b *strings.Builder, server *model.Server, theme *Theme, cfg *config.Config, indent string) {
	...
	if r.detail() != "minimal" {
		...
		if len(services) > 8 {
			b.WriteString(fmt.Sprintf("%s  grid-columns: 4\n", indent))
		}
...
func (r *D2Renderer) renderService(b *strings.Builder, svc *model.Service, theme *Theme, indent string) {
	...
	if svc.Type == model.ServiceTypeSystem && strings.HasPrefix(svc.Name, "system-summary-") {
		...
		b.WriteString(fmt.Sprintf("%s%s: %s {\n", indent, id, util.Quote(label)))
		b.WriteString(fmt.Sprintf("%s  style.fill: %q\n", indent, color.Fill))
		b.WriteString(fmt.Sprintf("%s  style.stroke: %q\n", indent, color.Stroke))
		b.WriteString(fmt.Sprintf("%s}\n", indent))
		return
	}
	...
	b.WriteString(fmt.Sprintf("%s%s: %s", indent, id, util.Quote(label)))
...
func (r *D2Renderer) renderDevices(b *strings.Builder, infra *model.Infrastructure, theme *Theme) {
	...
	for _, dev := range devices {
		...
		b.WriteString(fmt.Sprintf("    %s: %s", id, util.Quote(label)))
...
func (r *D2Renderer) renderExternalConnections(b *strings.Builder, infra *model.Infrastructure) {
	...
	if hasProduction {
		...
		b.WriteString(fmt.Sprintf("cloudflare: \"Cloudflare\" {\n  shape: cloud\n  style.fill: %q\n  style.stroke: %q\n}\n", cloudColor.Fill, cloudColor.Stroke))
		...
		for _, server := range infra.Servers {
			...
			b.WriteString(fmt.Sprintf("cloudflare -> %s\n", target))
		}
	}
		...
		for _, server := range infra.Servers {
			...
			for _, svc := range server.Services {
				...
				for _, dep := range svc.DependsOn {
					...
					if r.detail() == "detailed" {
						b.WriteString(fmt.Sprintf("tailnet.%s.%s.%s -> tailnet.%s.%s.%s: \"depends_on\" { style.stroke-dash: 3 }\n",
							groupID, sid, svcID, groupID, sid, depID))
					} else {
						b.WriteString(fmt.Sprintf("tailnet.%s.%s.%s -> tailnet.%s.%s.%s { style.stroke-dash: 3 }\n",
							groupID, sid, svcID, groupID, sid, depID))
					}
				}
			}
