...
func TestTailscaleCollector(t *testing.T) {
	...
	// Self (homelab) should be added as server (no server tags, but it's self)
	// Actually homelab has no tags so it becomes a device
	...
	// Devices: homelab (self, no server tag), user-phone, homeassistant
	assert.Contains(t, infra.Devices, "homelab")
